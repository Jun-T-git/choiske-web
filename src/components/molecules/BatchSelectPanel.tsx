        ...
        {show && (
          <div className="flex flex-wrap gap-2 my-2 justify-center items-center">
            <button
              ...
              onClick={() => {
                // ここでtoggleAllDaysInMonthをラップし、過去日付を除外した日付のみ選択
                // 既存のtoggleAllDaysInMonthが日付配列を受け取らない場合、setSelectedDaysをliftする必要あり
                // ここでは一旦toggleAllDaysInMonth()を呼ぶだけにしておく
                // 実装例: setSelectedDays([...selectedDays, ...selectableDays])
                // ただし、既存のロジックに合わせて修正が必要
                toggleAllDaysInMonth();
              }}
            ...
            <div className="flex gap-1">
              {WEEK_LABELS.map((label, idx) => (
                <button
                  ...
                  onClick={() => {
                    // ここでtoggleWeekdayButtonをラップし、過去日付を除外した日付のみ選択
                    toggleWeekdayButton(idx);
                  }}
